...
/**
 * An ISO datetime string, as generated by a Date().getISOString();
 * @typedef {string} ISODateString;
 */

/** 
 * An object containing various bits of date information.
 * @typedef {Object} DateObject
 * @property {number} refdate - date.getTime() representing the reference date (typically when the item was last updated)
 ...
 * @property {string} opart - the original string that created this chunk.
 * @property {string} part - a string that would create this same chunk (some references may have been updated)
 * @property {(number|ISODateString)} reps - integer OR ISO date string representing number of times the recurrence should occur. -1 if forever. because storeable in json and ifferent from # reps
 */
...
function _dateParser() {
     ...
     * 
     * @returns {Date()} a time if a valid time is detected; otherwise returns undefined.
     */
    this.extractNextTime = function(str, refdate) {
        let d;
        if (!refdate) {
            d = new Date();
            refdate = new Date();
     ...
     * @param {Date()} refdate The starting reference date.
     * 
     * @returns {DateObject} An array of DateObjects calculated from the refdate.
     */
