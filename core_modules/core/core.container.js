...
function loadTableData() {
    ...
    /*$.getscript("../../operators/itemList.js",function(){
        alert(get_table_data());
    });*/
    //alert(JSON.stringify(data_table[0].id));
    //alert(JSON.stringify(data_table[data_table.length-1]));
...
<table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0
 style='border-collapse:collapse;border:none'>`;
    ...
    $.each(data_table, function(i){
        data += `<tr>`;
        if(typeof data_table[i].title !== "undefined") {
            data += `
            <td width=201 valign=top style='width:151.0pt;border:solid windowtext 1.0pt;
            padding:0in 5.4pt 0in 5.4pt'>
            <p class=MsoNormal style='margin-bottom:0in;line-height:normal'>`;
            data += (data_table[i].title);
            data += `</p>
            </td>`}
        if(typeof data_table[i].importance !== "undefined") {
            data += `
            <td width=201 valign=top style='width:151.0pt;border:solid windowtext 1.0pt;
            padding:0in 5.4pt 0in 5.4pt'>
            <p class=MsoNormal style='margin-bottom:0in;line-height:normal'>`;
            data += (data_table[i].importance);
            data += `</p>
            </td>`}
        if(typeof data_table[i].date !== "undefined") {
            data += `
            <td width=201 valign=top style='width:151.0pt;border:solid windowtext 1.0pt;
            padding:0in 5.4pt 0in 5.4pt'>
            <p class=MsoNormal style='margin-bottom:0in;line-height:normal'>`;
            data += (data_table[i].date);
            data += `</p>
            </td>`};`
